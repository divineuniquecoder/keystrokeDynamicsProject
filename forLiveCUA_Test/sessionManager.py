...
from conAuthen import ContinuousAuthentication
import threading
...
class SessionManager:
    def __init__(self, data_collector, db_handler, task_manager, user_id):
        ...
        self.cont_auth = ContinuousAuthentication("svm11.pkl")
    ...
    def start_session(self, initial_call=True):
        if initial_call:
            if not self.current_session_active:
                self.current_session_active = True
                print("Session started.")
                self.start_session(initial_call=False)
        else:
            preprocessed_metrics = self.data_collector.prepare_metrics_for_authentication()
            if preprocessed_metrics is not None:
                prediction = self.cont_auth.predict(preprocessed_metrics)
                print(f"Authentication prediction: {prediction}")
            else:
                print("Insufficient data for authentication.")
            # Schedule the next session check after a delay (e.g., 60 seconds)
            threading.Timer(60, self.start_session, args=[False]).start()
    ...
    def save_to_text_file(self, user_id, session_id, task_type, processed_data, file_path='session_datatest.csv'):
    ...
    '''
    def start_authentication_check(self):
        if self.current_session_active:
            # Use the new preprocessing method to get scaled features ready for prediction
            user_metrics = self.data_collector.preprocess_and_scale_features()
            # Now use these preprocessed and scaled features for authentication
            prediction = self.cont_auth.authenticate(user_metrics)
            print(f"Authentication prediction: {prediction}")
            # Reschedule the next check
            threading.Timer(60, self.start_authentication_check).start()
